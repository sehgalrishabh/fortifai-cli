...
function countSeverity(
    ...
    return evaluationResults.reduce((acc, result) => {
        const count = result.deduplicatedSignals.filter(
...
function avgRiskScore(evaluationResults: EvaluationResult[]): number {
    if (evaluationResults.length === 0) return 0;
    const total = evaluationResults.reduce(
        (acc, result) => acc + result.compositeScore,
        0,
    );
    return Math.round(total / evaluationResults.length);
}
...
export function buildRequestOutcomes(scanResults: AttackResult[]): RequestOutcomeSummary {
...
export function buildLatencySummary(scanResults: AttackResult[]): LatencySummary {
...
export function buildSummary(
    ...
    return {
        ...
        findings: {
            CRITICAL: countSeverity(evaluationResults, "CRITICAL"),
            HIGH: countSeverity(evaluationResults, "HIGH"),
            MEDIUM: countSeverity(evaluationResults, "MEDIUM"),
            LOW: countSeverity(evaluationResults, "LOW"),
        },
        riskScore: avgRiskScore(evaluationResults),
...
export function printSummary(
    scanResults: AttackResult[],
    _evaluationResults: EvaluationResult[],
    scanId: string,
): void {
    ...
    console.log(`    Responded      : ${outcomes.successful}  (${asPercent(outcomes.successful, outcomes.total)})`);
    console.log(`    Timed Out      : ${outcomes.timedOut}  (${asPercent(outcomes.timedOut, outcomes.total)})`);
    console.log(`    Transport Err  : ${outcomes.transportErrors}  (${asPercent(outcomes.transportErrors, outcomes.total)})`);
    console.log();
    ...
    console.log(`    Avg / P50      : ${summary.latency.avgMs} / ${summary.latency.p50Ms}`);
    console.log(`    P95 / P99      : ${summary.latency.p95Ms} / ${summary.latency.p99Ms}`);
    console.log(`    Min / Max      : ${summary.latency.minMs} / ${summary.latency.maxMs}`);
    console.log();

    console.log(`  Security Evaluation  : pending — results on dashboard after upload`);
    console.log(line);
    console.log();
}
